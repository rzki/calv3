            ...
            <div class="col-12 col-xl-12">
                <div class="px-0 col-12">
                    <div class="border-0 shadow card">
                        <div class="card-body">
                            <h2 class="mb-1 fs-5 fw-bold mb-3">{{ __('Semua QR Alat Terisi') }}</h2>
                            {{-- <div class="row mb-4">
                                                        ...
                                                        @if (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Teknisi'))
                                                            <div class="col-lg-6">
                                                                ...
                                                                <input type="date" name="start-date" id="start-date"
                                                                    class="form-control" wire:model='start_date_admin'>
                                                            </div>
                                                            <div class="col-lg-6">
                                        ...
                                        <table class="table striped-table text-black text-center">
                                            ...
                                            <tbody>
                                                @if (Auth::user()->hasRole(['Superadmin', 'Admin']))
                                                    ...
                                                    @endif
                                                    @foreach ($alatAdmin as $device)
                                                        <tr>
                                                            <td>{{ $alatAdmin->firstItem() + $loop->index }}</td>
                                                ...
                                                @else
                                                    ...
                                                    @foreach ($alats as $device)
                                                        <tr>
                                                            ...
                                                            <td>{{ $device->users->name ?? '' }}</td>
                                                            {{-- <td>
                                                            <a href="{{ route('devices.detail', $device->deviceId) }}" class="btn btn-primary" target="_blank"><i
                                                                    class="fas fa-eye"></i></a>
                                                            <a class="btn btn-secondary" target="_blank" wire:click="print('{{ $device->deviceId }}')"><i
                                                                    class="fas fa-print"></i></a>
                                                            <a href="{{ route('devices.edit', $device->deviceId) }}" class="btn btn-info"><i
                                                                    class="fas fa-pen-to-square"></i></a>
                                                            <button class="btn btn-danger" wire:click.prevent="deleteConfirm('{{ $device->deviceId }}')"><i
                                                                    class="fas fa-trash"></i></button>
                                                        </td> --}}
                                                        </tr>
                                                    @endforeach
