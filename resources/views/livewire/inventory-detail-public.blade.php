<div>
    <div class="row vh-100 mx-0 my-4">
        <div class="col d-flex flex-column justify-content-center align-items-center">
            @if (Auth::user())
            <div class="row mb-5">
                <a href="{{ route('inventories.index') }}" wire:navigate class="btn btn-primary"><i
            ...
            <div class="row mb-4">
                @if (Auth::guest())
                <a href="{{ route('devices.detail', $qr->deviceId) }}" class="btn btn-danger btn-block w-100">{{
                    __('Masuk untuk mengubah data') }}</a>
                {{-- <a href="{{ route('login') }}" class="btn btn-info"><i class="fas fa-up-right-from-square"></i> {{
                    __('Lihat Sertifikat') }}</a> --}}
                <script>
                    sessionStorage.setItem('intended_url', route('devices.publicDetail', $qr->deviceId));
                </script>
                @elseif(Auth::check())
                <a href="{{ route('devices.edit', $qr->deviceId) }}" class="btn btn-success w-100">{{ __('Perbarui
                    Data') }}</a>
                @endif
            </div>
        </div>
